...
initial_balance = get_balance("KRW")  # 초기 잔액 저장
    ...
    try:
        ...
        if len(buy_conditions) >= 3:  # 매수 조건 중 최소 3개 이상 충족 시
            ...
            if krw > 5000 and krw >= investment_per_coin:
                ...
                post_message(myToken, slackchannel, f"{ticker} 매수 완료: {buy_result}, 조건: {', '.join(buy_conditions)}")
        ...
        if crypto_balance > 0.00008 and buy_price is not None:
            ...
            if len(sell_conditions) >= 3:  # 매도 조건 중 최소 3개 이상 충족 시
                ...
                post_message(myToken, slackchannel, f"{ticker} 매도 완료: {sell_result}, 조건: {', '.join(sell_conditions)}\n수익: {sell_profit:.2f} KRW\n수익률: {sell_profit_rate:.2f}%")
...
# 종료 시 슬랙 알림 보내기
def on_exit():
    final_balance = get_balance("KRW")
    total_profit, total_profit_rate = calculate_total_profit(initial_balance, final_balance)
    post_message(myToken, slackchannel, f"!!업비트 자동매매 종료!!\n초기 잔액: {initial_balance} KRW\n최종 잔액: {final_balance} KRW\n총 수익: {total_profit} KRW\n총 수익률: {total_profit_rate:.2f}%")
