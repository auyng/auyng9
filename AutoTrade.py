...
def get_balance():
    """잔고 조회 (캐시 사용)"""
    global cache
    if cache['balance'] is None or (datetime.datetime.now() - cache['balance_time']).seconds > 60:
        balance = binance.fetch_balance()
        cache['balance'] = balance['total']['USDT']
        cache['balance_time'] = datetime.datetime.now()
    return cache['balance']

def get_current_price(symbol):
    """현재가 조회 (캐시 사용)"""
    global cache
    if cache['current_price'] is None or (datetime.datetime.now() - cache['current_price_time']).seconds > 60:
        ticker = binance.fetch_ticker(symbol)
        cache['current_price'] = ticker['last']
        cache['current_price_time'] = datetime.datetime.now()
    return cache['current_price']

def fetch_ohlcv(exchange, symbol, timeframe=timedata):
    """OHLCV 데이터 조회 (캐시 사용)"""
    global cache
    if cache['ohlcv'] is None or (datetime.datetime.now() - cache['ohlcv_time']).seconds > 60:
        cache['ohlcv'] = exchange.fetch_ohlcv(symbol=symbol, timeframe=timeframe, limit=100)
        cache['ohlcv_time'] = datetime.datetime.now()
    return cache['ohlcv']

def get_rsi(exchange, symbol, period=6):
    """RSI (Relative Strength Index) 조회"""
    coin = fetch_ohlcv(exchange, symbol)
...
def fetch_position(exchange, symbol):
    """현재 포지션 조회 (캐시 사용)"""
    global cache
    if cache['position'] is None or (datetime.datetime.now() - cache['position_time']).seconds > 60:
        positions = exchange.fapiPrivateGetPositionRisk({'symbol': exchange.market(symbol)['id']})
        for pos in positions:
            if float(pos['positionAmt']) != 0:
                ...
                cache['position'] = {
                    ...
                    "initial_balance": get_balance()  # 초기 잔고를 현재 잔고로 설정
                }
                break
        else:
            cache['position'] = {
        ...
        cache['position_time'] = datetime.datetime.now()
...
def enter_position(exchange, symbol, cur_price, amount):
    ...
    # 현재 포지션 상태 동기화
    current_position = fetch_position(exchange, symbol)
    if current_position['type'] != position['type'] or current_position['amount'] != position['amount'] or current_position['entry_price'] != position['entry_price']:
        position.update(current_position)
        post_message(myToken, slackchannel, f"포지션 상태 동기화: {position}")
